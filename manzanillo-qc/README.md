...
├── scaling_plots/                generated by scaling benchmark (created on first run)
└── src/manzanillo_qc/
...
- Uses Pasqal's `qubo-solver` library with calibrated λ (`build_qubo_sampling`)
...
- **Cloud mode** (set env vars → `RemoteEmulator` on Pasqal Cloud):
  ```bash
  export PASQAL_USERNAME="your@email.com"
  export PASQAL_PASSWORD="yourpassword"
  export PASQAL_PROJECT_ID="..."   # optional, default project hard-coded
  ```
- **Local fallback** (no credentials needed): QutipBackendV2 (n<16) or MPSBackend / emu-mps (n≥16, χ=256)
...
|---------------------|------------------------------------------|---------------------------------|
| `QUBIT_COUNTS`      | [4, 6, 8, 10, 12, 14]                    | Default sweep                   |
| `BUDGET_10K`        | 80                                       | $800K — ~40% selection ratio    |
| `DEFAULT_P`         | 6                                        | QAOA circuit depth              |
| `DEFAULT_STEPS`     | 300                                      | Adam iterations                 |
